 ...
 * 
 * 常见误解：
 * "如果高16位全0，左移后会丢失低16位的1，导致计数错误"
 * 
 * 正确理解：
 * 1. 本函数计算的是"从最高位开始的连续0的个数"
 * 2. 不是计算"总共有多少个0"
 * 3. 一旦遇到第一个1，计数就停止
 * 
 * 算法逻辑澄清：
 * 
 * 当 !(num & 0xFFFF0000) 时（即高16位全0）：
 * - 说明高16位全是0
 * - 这16个0必须计入前导零的计数
 * - 左移丢弃这16个0，专注于剩余位
 * - 不关心低16位有什么1，只关心还有多少连续的前导零
 * 
 * 例子说明：
 * num = 0x00001234 = 00000000000000000001001000110100
 *       ↑
 *   16个连续的前导零
 * 
 * 执行过程：
 * 1. 高16位全0 → count=16, num左移16位变成0x12340000
 * 2. 高8位非0 → 停止，不再检查
 * 3. 结果：16个前导零 ✓
 * 
 * 关键：我们不需要保留低位的1，因为：
 ...
 * 
 * 对称性理解：
 * - count_leading_zeros：从左向右数0，用左移丢弃已确认的0段
 * - count_trailing_zeros：从右向左数0，用右移丢弃已确认的0段
 * - 两者都是"跳过连续0的段"，不是"计算总0个数"
 */
 ...
 * 
 * 常见误解：
 * "如果低16位全0，右移后会丢失高16位的1，导致计数错误"
 * 
 * 正确理解：
 * 1. 本函数计算的是"从最低位开始的连续0的个数"
 * 2. 不是计算"总共有多少个0"
 * 3. 一旦遇到第一个1，计数就停止
 * 
 * 算法逻辑澄清：
 ...
 * - 这16个0必须计入末尾零的计数
 * - 右移丢弃这16个0，专注于剩余位
 * - 不关心高16位有什么1，只关心还有多少连续的0
 * 
 * 例子说明：
 * num = 0x12340000 = 00010010001101000000000000000000
 *                                    ↑
 *                              16个连续的末尾零
 * 
 * 执行过程：
 * 1. 低16位全0 → count=16, num=0x1234
 * 2. 低8位非0 → 停止，不再检查
 * 3. 结果：16个末尾零 ✓
 * 
 * 关键：我们不需要保留高位的1，因为：
 ...
 * 
 * 常见误解：
 * "右移31位应该得到0或1（符号位），为什么是0或-1？"
 * 
 * 正确理解：
 * 1. C语言中，右移操作符>>的行为取决于操作数类型：
 *    - 有符号整数：算术右移（符号扩展）
 *    - 无符号整数：逻辑右移（零填充）
 * 
 * 2. 算术右移的符号扩展机制：
 ...
 * 
 * 3. 具体例子：
 *    int x = -5;  // 11111111111111111111111111111011
 *    x >> 31;     // 11111111111111111111111111111111 = -1 (不是1！)
 *    
 *    int y = 5;   // 00000000000000000000000000000101
 *    y >> 31;     // 00000000000000000000000000000000 = 0
 * 
 * 4. 为什么需要-1而不是1：
 *    -1 = 0xFFFFFFFF 是完美的"全1掩码"：
 *    - value ^ (-1) = ~value  (按位取反)
 *    - value & (-1) = value   (保持原值)
 *    - 这正是算法需要的掩码特性
 * 
 ...
 *    
 *    signed_val >> 31;    // 算术右移 → -1
 *    unsigned_val >> 31;  // 逻辑右移 → 1
 *    
 *    相同的>>操作符，不同的类型，不同的结果！
 * 
 * 6. 可移植性考虑：
